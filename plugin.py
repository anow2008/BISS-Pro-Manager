...
class BISSPro(Screen):
    def __init__(self, session):
        ...
        self.skin = """
        <screen position="center,center" size="1100,750" title="BissPro Manager v4.0">
    ...
    # وظيفة حفظ موحدة تستخدم SID + VPID
    def unified_save(self, key, name):
        service = self.session.nav.getCurrentService()
        if not service: return False
        info = service.info()
        
        # استخراج البيانات بنفس طريقة اليدوي
        ...
        # المعرف المدمج المشترك (SID+VPID)
        combined_id = (hex_sid + hex_vpid).upper()
        target = get_softcam_path()
        
        try:
            os.system("chmod 644 %s" % target)
            # حذف أي مدخل قديم بنفس المعرف
            os.system("sed -i '/F %s/d' %s" % (combined_id.zfill(8), target))
            
            # إضافة السطر الجديد بنظام 8 أرقام للمعرف
            new_line = "F %s 00000000 %s ;%s" % (combined_id.zfill(8), key.upper(), name)
    ...
    def ok(self):
        ...
        if curr and curr[0] == "add": 
            ...
            self["status"].setText("Smart Syncing with biss.txt...")
    ...
    def manual_done(self, key=None):
        ...
        if res_id:
            self.res = (True, "Manual Save Success: %s" % res_id)
        else:
    ...
    def do_auto(self):
        try:
            ...
            info = service.info()
            raw_sid = info.getInfo(iServiceInformation.sSID) & 0xFFFF
            sid_hex_search = "%X" % raw_sid # للبحث المرن
            
            # تحميل الملف
            url = "https://raw.githubusercontent.com/anow2008/softcam.key/main/biss.txt"
            data = urlopen(url, timeout=10).read().decode("utf-8")
            
            # البحث عن الشفرة بـ SID فقط من الملف
            pattern = r'(?:^|[\s,])0*%s[:\s=]+([0-9A-Fa-f]{16})' % sid_hex_search
            m = re.search(pattern, data, re.I)
            
            if m:
                found_key = m.group(1).upper()
                # هنا السر: نمرر الشفرة لوظيفة الحفظ الموحدة لتدمجها مع الـ VPID الخاص بالقناة الحالية
                res_id = self.unified_save(found_key, info.getName())
                if res_id:
                    self.res = (True, "Auto Linked: %s\nKey: %s" % (res_id, found_key))
            else:
                self.res = (False, "SID %04X Not Found in Online List" % raw_sid)
        except:
            self.res = (False, "Internet/Server Error")
        self.timer.start(100, True)
...
class HexInputScreen(Screen):
    def __init__(self, session):
        Screen.__init__(self, session)
        self.skin = """
        <screen position="center,center" size="900,550" title="BISS Editor" backgroundColor="#121212">
...
def Plugins(**kwargs): return [PluginDescriptor(name="BissPro", description="v4.0 Unified Data System", icon="plugin.png", where=PluginDescriptor.WHERE_PLUGINMENU, fnc=main)]
