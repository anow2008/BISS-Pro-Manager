...
class BISSPro(Screen):
    def __init__(self, session):
        ...
        self.skin = f"""
        <screen position="center,center" size="{self.ui.px(1100)},{self.ui.px(750)}" title="BissPro Manager v2.4">
    ...
    def build_menu(self):
        ...
        for text, action, icon_name in items:
            ...
            # استخدام LoadPixmap بطريقة تضمن المسار الصحيح
    ...
    def do_auto(self, service):
        try:
            ...
            if m and self.save_key(sid, m.group(1), info.getName()): self.res = (True, "Key Found!")
...
def Plugins(**kwargs):
    return [PluginDescriptor(name="BissPro", description="BISS Manager v2.4", icon="plugin.png", where=PluginDescriptor.WHERE_PLUGINMENU, fnc=main)]
