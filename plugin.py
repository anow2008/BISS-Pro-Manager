...
class BISSPro(Screen):
    ...
    def build_menu(self):
        lst = []
        # ترتيب العناصر: النص الرئيسي، ثم الوصف، ثم الرمز البرمجي كبيانات إضافية
        menu_items = [
        ...
        for title, desc, action in menu_items:
            # نضع النص أولاً في القائمة لضمان ظهوره بشكل صحيح
            item = [title] 
            item.append(MultiContentEntryText(pos=(self.ui.px(20), self.ui.px(10)), size=(self.ui.px(950), self.ui.px(50)), font=0, text=title, flags=RT_VALIGN_TOP))
            item.append(MultiContentEntryText(pos=(self.ui.px(20), self.ui.px(60)), size=(self.ui.px(950), self.ui.px(40)), font=1, text=desc, flags=RT_VALIGN_TOP, color=0xbbbbbb))
            item.append(action) # الرمز البرمجي يكون في الأخير كمرجع
            lst.append(item)
    ...
    def ok(self):
        ...
        if curr:
            act = curr[-1] # نأخذ الرمز من آخر القائمة
...
class BissManagerList(Screen):
    def __init__(self, session):
        ...
        <screen position="center,center" size="{self.ui.px(1000)},{self.ui.px(700)}" title="BissPro - Key Editor">
            ...
            <eLabel text="GREEN: Edit | RED: Delete" position="{self.ui.px(75)},{self.ui.px(585)}" size="{self.ui.px(600)},{self.ui.px(40)}" font="Regular;26" transparent="1" />
        </screen>"""
    ...
    def finish_edit(self, new_key=None):
        if new_key is None: return # تم إصلاح خطأ الـ missing positional argument
...
class HexInputScreen(Screen):
    def __init__(self, session, channel_name="", existing_key=""):
        ...
        <screen position="center,center" size="1000,600" title="Key Input" backgroundColor="#1a1a1a">
            ...
            <eLabel text="GREEN: Save | RED: Exit" position="10,540" size="980,40" font="Regular;24" halign="center" transparent="1" />
        </screen>"""
        self["channel"] = Label(f"{channel_name}"); self["keylabel"] = Label(""); self["progress"] = ProgressBar()
