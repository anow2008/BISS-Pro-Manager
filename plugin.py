...
class BISSPro(Screen):
    def __init__(self, session):
        ...
        self.skin = f"""
        <screen position="center,center" size="{self.ui.px(1100)},{self.ui.px(750)}" title="BissPro Manager v1.6">
    ...
    def build_menu(self):
        ...
        for text, action, icon in items:
            ...
            lst.append((action, [MultiContentEntryPixmapAlphaTest(pos=(self.ui.px(10), self.ui.px(10)), size=(self.ui.px(100), self.ui.px(100)), png=LoadPixmap(p)),
    ...
    def ok(self):
        ...
        if action == "add" and service: self.session.openWithCallback(self.manual_done, HexInputScreen)
        elif action == "upd":
    ...
    def manual_done(self, key=None):
        if not key: return # إذا خرج بـ Exit (None) لا تفعل شيئاً
        info = self.session.nav.getCurrentService().info()
        sid = "%08X" % info.getInfo(iServiceInformation.sSID)
        if self.save_key(sid, key, info.getName()): self.res = (True, "BISS Key Saved")
    ...
    def do_auto(self, service):
        try:
            ...
            if m and self.save_key(sid, m.group(1), info.getName()): self.res = (True, "Auto-Added!")
